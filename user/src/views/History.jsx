                ...
                <div className='flex'>
                  ...
                  <div className={`flex flex-col items-start text-center min-h-32 p-5`}>
                    ...
                    <div className="text-start">
                      <p className='my-auto'>Tiền thanh toán: <b>{formatNumber(item.thanhTien + (listInvoice.find((invoice) => invoice.datSan._id == item._id)?.phuThu || 0) - (listInvoice.find((invoice) => invoice.datSan._id == item._id)?.giamGia || 0))}</b></p>
                    </div>
                    {/* {listInvoice.find((invoice) => invoice.datSan._id == item._id) ? (
                      <div className="text-start">
                        <p className='my-auto'>Phụ thu: <b>{formatNumber(listInvoice.find((invoice) => invoice.datSan._id == item._id)?.phuThu || 0)}</b></p>
                      </div>
                    )
                      : ''} */}
