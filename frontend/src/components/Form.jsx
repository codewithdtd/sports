          ...
          <form onSubmit={handleSubmit} className='flex flex-col items-center'>
            {props.name == 'signUp' ?
              <div className="flex sm:w-full lg:w-[50%]">
                <div className="flex form-items bg-[#C6FFC8] sm:w-full lg:w-[50%] mr-2">
                  <i className="ri-user-3-line"></i>
                  <input className={`bg-[#C6FFC8]`} type="text" name="ho_NV" value={user.ho_NV} onChange={(e)=>setUser({...user, ho_NV: e.target.value})} placeholder='Họ và tên đệm' required/>
                </div>
                <div className='form-items bg-[#C6FFC8] sm:w-[100%] lg:w-[50%]'>  
                  <i className="ri-user-3-line"></i>
                  <input className={`bg-[#C6FFC8]`} type="text" name="ten_NV" value={user.ten_NV} onChange={(e)=>setUser({...user, ten_NV: e.target.value})} placeholder='Tên' required/>
                </div>
              </div> : ''
            }
            <div className='form-items bg-[#C6FFC8] sm:w-[100%] lg:w-[50%]'>
              <i className="ri-phone-line"></i>
              <input className={`bg-[#C6FFC8]`} type="text" name="sdt_NV" value={user.sdt_NV} onChange={(e)=>setUser({...user, sdt_NV: e.target.value})} placeholder='Số điện thoại' required/>
            </div>
            { props.name == 'signUp' ?
              <div className="form-items bg-[#C6FFC8] sm:w-[100%] lg:w-[50%]">
                <i className="ri-mail-line"></i>
                <input className={`bg-[#C6FFC8]`} type="text" name="email_NV" value={user.email_NV} onChange={(e)=>setUser({...user, email_NV: e.target.value})} placeholder='Email (Nếu có)'/>
              </div> : ''
            }
            <div className="form-items bg-[#C6FFC8] sm:w-[100%] lg:w-[50%]">
              <i className="ri-lock-line"></i>
              <input className={`bg-[#C6FFC8]`} type="password" name="matKhau_NV" value={user.matKhau_NV} onChange={(e)=>setUser({...user, matKhau_NV: e.target.value})} placeholder='Mật khẩu' required/>
            </div>

            { props.name == 'signUp' ?
              <div className='sm:w-[100%] lg:w-[50%]'>
                <div className={`form-items ${checkPassword ? 'bg-red-200' : 'bg-[#C6FFC8]'}`}>
                  <i className="ri-lock-line"></i>
                  <input className={`${checkPassword ? 'bg-red-200' : 'bg-[#C6FFC8]'}`} type="password" name="matKhau_NV" value={user.confirm_matKhau_NV} onChange={(e)=>setUser({...user, confirm_matKhau_NV: e.target.value})} placeholder='Xác nhận mật khẩu' required/>
                </div> 
